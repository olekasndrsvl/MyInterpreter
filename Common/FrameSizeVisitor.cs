...
{
    private readonly List<string> _alreadyGeneratedFunctions = new();
    private NameSpace _currentNameSpace;
    private readonly Dictionary<string, int> _frameSizes = new();
    private readonly Stack<string> _functionStack = new();
    private readonly Stack<FunctionSpecialization> _specializationStack = new();
    ...
    public FrameSizeVisitor()
    {
        ...
        var initialTemps = SymbolTree.FunctionTable["Main"].Specializations.First().NameSpace.Variables
    ...
    public void VisitBinOp(BinOpNode bin)
    {
        ...
        var leftTemp = _tempCounters[_functionStack.Peek()] - 1;

        // Правый операнд
        bin.Right.VisitP(this);
        var rightTemp = _tempCounters[_functionStack.Peek()] - 1;
        ...
        else if (leftType == SemanticType.DoubleType && rightType == SemanticType.IntType) NewTemp(); // convertedTemp
    ...
    public void VisitAssign(AssignNode ass)
    {
        ...
        if (ass.Expr is IntNode || ass.Expr is DoubleNode) return;
    ...
    public void VisitAssignOp(AssignOpNode ass)
    {
        ...
        if (varType == SemanticType.DoubleType && exprType == SemanticType.IntType) NewTemp(); // convertedTemp
    ...
    {
        foreach (var param in p.Pars.lst) param.VisitP(this);
    }

    public void VisitFuncCall(FuncCallNode f)
    {
        // Параметры
        foreach (var param in f.Pars.lst) param.VisitP(this);
    ...
    {
        foreach (var def in defList.lst) def.VisitP(this);
    }
    ...
    public void VisitReturn(ReturnNode r)
    {
        ...
        var expectedType = _specializationStack.Peek().ReturnType;

        if (returnType == SemanticType.IntType && expectedType == SemanticType.DoubleType) NewTemp(); // convertedTemp
    ...
    private int NewTemp()
    {
        var funcName = _functionStack.Peek();
        var current = _tempCounters[funcName];
        _tempCounters[funcName] = current + 1;

        // Обновляем размер фрейма если нужно
        if (_tempCounters[funcName] > _frameSizes[funcName]) _frameSizes[funcName] = _tempCounters[funcName];
    ...
    {
    } // Не нужно для подсчета размеров
}